...
CREATE TABLE TEAM(    -- 부서/팀(TEAM)
	-- TEAM_NO INT PRIMARY KEY AUTO_INCREMENT,
	TEAM_NAME VARCHAR(100) NOT NULL, -- 팀명
    ROLE VARCHAR(300), -- 업무 역할
    GOAL INT, -- 목표 실적
    LEADER VARCHAR(50) REFERENCES USER_INFO(USER_NO), -- 관리자(정)
    VICE VARCHAR(50) REFERENCES USER_INFO(USER_NO), -- 관리자(부)
    LOCATION VARCHAR(100) -- 근무지
    
);
CREATE TABLE GRADE(         -- 직급
	-- GRADE_NO INT PRIMARY KEY AUTO_INCREMENT,
	GRADE_NAME VARCHAR(100) NOT NULL, -- 직급명
    BONUS_RATE INT -- 보너스(%)
);

CREATE TABLE SCHEDULE(      -- 일정관리
    -- SCHEDULE_NO INT PRIMARY KEY AUTO_INCREMENT,
    HIRE_DATE DATE REFERENCES USER_INFO(HIRE_DATE), -- 입사 일자
    QUIT_DATE DATE REFERENCES USER_INFO(QUIT_DATE), -- 퇴사 일자
    DAY_CHECK INT, -- 근속 일수
    VACATION DATE, -- 휴가 일정
    VAC_LEFT INT, -- 남은 휴가 일수
    WORK_TYPE VARCHAR(100), -- 근무 형태
    WORK_TIME VARCHAR(100) -- 근무 시간(범위)
);

CREATE TABLE RESOURCE(     -- 자원관리
    -- RESOURCE_NO INT PRIMARY KEY AUTO_INCREMENT,
    RESOURCE_TYPE VARCHAR(100) NOT NULL, -- 자원 종류
    RESOURCE_NAME VARCHAR(100) NOT NULL, -- 자원명
    TOTAL_AMOUNT INT, -- 총량
    AMO_BUY INT, -- 구입량
    AMO_SELL INT, -- 판매량
    AMO_PRODUCE INT, -- 생산량
