...
CREATE TABLE FINANCIAL_BUDGET(
    ...
    ANNUAL_PLAN TEXT,  -- 각 부서별 예산 소진 계획
...
CREATE TABLE FINANCIAL_TRANSACTION(
    ...
    TRANS_AMOUNT INT,
    TRANS_DESC TEXT NOT NULL, -- 지출 및 수입 내역 상세정보
    TRANS_DATE TEXT NOT NULL, -- 지출 및 수입 내역 발생 일자
    USER_NO INT
);
...
CREATE TABLE QUALITY(
    ...
    STATUS VARCHAR(30) CHECK (STATUS IN('정상', '불량', '조치 필요')),
    USER_NO INT  -- 담당자(관리)
);
...
-- QUALITY_DEFECT : USER_NO, QUALITY_NO
...
-- ** MySQL [DATABASE] 메뉴 내 Reverse Engineering (Ctrl+R) 을 통해서 
-- 진행 중인 페이지 내 테이블 및 컬럼 구성을 관계도 형식으로 조회할 수 있다.

-- ** SCHEMA 접속은 좌측 메뉴에서 더블 클릭을 통해 BOLD가 되면 활성화된 상태