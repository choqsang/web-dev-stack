/*
    ...
    - DBMS : Database Management System의 약자, 데이터베이스 관리 시스템 (ex: Oracle, MySQL 등)
...
SELECT NAME, AGE, HOBBY FROM USER_INFO; -- 오라클에서는 모두 대문자로 사용
SELECT name, age, hobby FROM user_info; -- MySQL 규칙
...
/*
    ...
    컬럼 별칭
    - 산술연산을 하게 되면 컬럼명이 지저분해진다. 이 때 컬럼명에 별칭을 부여하면 깔끔하게 보일 수 있다.
...
/*
    ...
    연결 연산자 : ||
    - 여러 컬럼값들을 마치 하나의 컬럼인것처럼 연결 ('문자열')
    - 컬럼값과 리터럴도 연결할 수 있음
*/
-- USER_INFO에서 이름(NAME), 나이(AGE), 단위(살) 조회
SELECT NAME "이름", AGE||'살' "나이" FROM USER_INFO;

-- USER_INFO에서 이름(NAME)의 MBTI는 XXXX입니다 조회
SELECT * FROM USER_INFO;
SELECT NAME||'의 MBTI는 '||MBTI||'입니다' "성격 유형" FROM USER_INFO;

/*
    DISTINCT
    - 컬럼에 중복된 값들을 한 번씩만 표시하고자 할 때
    - 주의사항 : DISTINCT는 SELECT 절에서 한 번만 기술 가능 (ONLY)
*/
