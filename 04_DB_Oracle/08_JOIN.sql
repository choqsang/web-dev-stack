/*
    ...
    - 관계형 데이터베이스는 최소한의 데이터로 각각의 테이블에 담고 있음
      ...
      만약 어떤 사원이 어떤 부서에 속해있는 지 부서명과 같이 조회하고 싶다면?
      => 즉, 관계형 데이터베이스에서 SQL문을 이용한 테이블 간에 "관계"를 맺어 원하는 데이터를 조회하는 방법

    "오라클 구문"(예전 버전, 다른 RDBMS에서도 쓰인다)과
    "ANSI 구문"(미국국립표준협회 == 산업표준을 제정하는 단체)
*/
/*
    ...
    SELECT 컬럼, 컬럼, ...
    FROM 테이블1, 테이블2
    WHERE 테이블1.컬럼 = 테이블2.컬럼; (PRIMARY-FOREIGN KEY)
...
-- ANSI 구문
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE JOIN DEPARTMENT ON (EMPLOYEE.DEPT_CODE = DEPARTMENT.DEPT_ID); -- (DEPT_CODE = DEPT_ID);
