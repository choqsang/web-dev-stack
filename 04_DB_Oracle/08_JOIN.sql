...
-- 컬럼명이 동일한 경우 USING 사용 추천!
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE);

-- 자연조인(NATURAL JOIN) : 각 테이블마다 동일한 컬럼이 한 개만 존재할 경우 (※ 추천하는 기능은 아니다.)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
...
FROM EMPLOYEE JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE NOT DEPT_TITLE = '총무부'; -- 조건에 != 를 써도 무방하다.
...
-- 사원명, 부서명 조회
-- ANSI 구문
-- JOIN을 기준으로 기준이 되는 테이블의 방향을 지정 (EMPLOYEE : 왼쪽 LEFT) // 사원은 존재하되 부서명이 NULL인 경우에도 조회
SELECT EMP_NAME "사원명", DEPT_TITLE "부서명"
FROM EMPLOYEE LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE);
-- RIGHT JOIN (DEPARTMENT : 오른쪽 RIGHT) // 부서는 존재하되 사원명이 NULL인 경우에도 조회
SELECT EMP_NAME "사원명", DEPT_TITLE "부서명"
FROM EMPLOYEE RIGHT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE);
-- FULL JOIN (LEFT + RIGHT : 두 테이블 모두 기준으로 삼음) // 각 테이블 내 값이 NULL인 경우 모두를 포함하여 조회
SELECT EMP_NAME "사원명", DEPT_TITLE "부서명"
FROM EMPLOYEE FULL JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE);

-- 오라클 구문 // 기준 삼는 테이블의 반대 테이블의 컬럼 뒤에 (+) // 플러스 기호는 FULL-JOIN은 불가하다! 한 군데에만 가능
...
/*
    자체 조인(SELF JOIN)
    - 같은 테이블을 다시 한 번 조인하는 경우 (자기 자신과 조인) => ex)대댓글
*/
-- 사원번호, 사원명, 부서코드, 사수사번, 사수명, 사수부서코드 조회
SELECT * FROM EMPLOYEE;
SELECT E.EMP_ID "사원번호", E.EMP_NAME "사원명", E.DEPT_CODE "부서코드", 
...
SELECT * FROM JOB;
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE CROSS JOIN DEPARTMENT; -- 23 X 9
...
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE) LEFT JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE) LEFT JOIN NATIONAL USING(NATIONAL_CODE);
...
FROM EMPLOYEE
LEFT JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE) LEFT JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
WHERE EMP_NO LIKE '7%' AND EMP_NO LIKE '%-2%' AND EMP_NAME LIKE '전%'; -- LIKE '7_____-2%'
...
SELECT NVL(DEPT_TITLE, '부서없음') "부서명", 
TO_CHAR(ROUND(AVG(SALARY)),'999,999,999') "평균 급여"
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
GROUP BY DEPT_TITLE;
...
DECODE(SAL_LEVEL,'S1','고급','S2','고급','S3','중급','S4','중급','초급') "구분"
/* CASE WHEN SAL_LEVEL IN('S1','S2') THEN '고급'
        WHEN SAL_LEVEL IN('S3','S4') THEN '중급'
        WHEN SAL_LEVEL IN('S5','S6') THEN '초급' END 구분 */ -- 디코드가 아닌 케이스로 할 경우
FROM EMPLOYEE
LEFT JOIN JOB USING(JOB_CODE)
LEFT JOIN SAL_GRADE ON SALARY BETWEEN MIN_SAL AND MAX_SAL
ORDER BY SAL_LEVEL;
...
FROM MOVIE
JOIN MOVIE_ACTOR USING(MOVIE_ID) JOIN ACTOR USING(ACTOR_ID);
...
-- 13. 사용자별 리뷰 수 조회
SELECT NAME 사용자, COUNT(USER_ID) "리뷰 수"
FROM REVIEW
JOIN USER_INFO USING (USER_ID)
GROUP BY NAME;

-- 14. 최동훈 감독의 영화에 출연한 배우 조회
SELECT DIRECTOR||' 감독의 '||TITLE||' 작품에 출연한 배우는 '||NAME||'입니다.' "배우 조회" -- DISTINCT NAME으로 배우만 확인가능
FROM ACTOR
JOIN MOVIE_ACTOR USING (ACTOR_ID) JOIN MOVIE USING (MOVIE_ID)
WHERE DIRECTOR='최동훈';

-- 15. 하정우가 출연한 영화 제목과 평균 평점 조회
SELECT TITLE "영화 제목", AVG(RATING) "평균 평점"
FROM REVIEW
JOIN MOVIE USING (MOVIE_ID)
JOIN MOVIE_ACTOR USING (MOVIE_ID) JOIN ACTOR USING (ACTOR_ID)
WHERE NAME='하정우'
...
SELECT ACTOR_ID FROM ACTOR WHERE NAME = '하정우'; -- 코드3
SELECT MOVIE_ID FROM MOVIE_ACTOR WHERE ACTOR_ID = 3; -- 하정우가 출연한 영화 (9,15,37,38,43,45)

SELECT TITLE, AVG(RATING)
FROM REVIEW JOIN MOVIE USING(MOVIE_ID)
-- WHERE MOVIE_ID IN(9,15,37,38,43,45) // 위의 찾는 식을 풀어서 대입하면 아래 서브쿼리와 같다.
WHERE MOVIE_ID IN (SELECT MOVIE_ID FROM MOVIE_ACTOR WHERE ACTOR_ID =(SELECT ACTOR_ID FROM ACTOR WHERE NAME = '하정우'))
GROUP BY TITLE;

-- 16. MBTI별 평균 평점 조회
SELECT MBTI, ROUND(AVG(RATING),1) "평균 평점"
FROM REVIEW
JOIN USER_INFO USING (USER_ID)
GROUP BY MBTI;

-- 17. 각 영화별 리뷰 수와 평균 평점 조회 (리뷰 수가 1건이라도 있는 경우부터)
SELECT TITLE "영화", COUNT(MOVIE_ID) "리뷰 수", AVG(RATING) "평균 평점"
FROM REVIEW
...
SELECT NAME "배우", COUNT(TITLE) "작품 수"
FROM MOVIE
JOIN MOVIE_ACTOR USING (MOVIE_ID) JOIN ACTOR USING (ACTOR_ID)
GROUP BY NAME
HAVING COUNT(NAME)>=3;