/*
    ...
    : 테이블(TABLE), 뷰(VIEW: 가상의 테이블), 시퀀스(SEQUENCE), 인덱스(INDEX), 
...
-- INSERT INTO 테이블명 VALUES(값, 값, 값,...); -- 테이블 내 컬럼 순서대로 값 기재
INSERT INTO MEMBER VALUES(
    1, 'user01', 'pass01', '윤의진', '남', '010-1111-2222', 'aaa@naver.com', '25/06/05'
);
INSERT INTO MEMBER(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME) VALUES(
    2, 'user02', 'pass02', '이진용');       -- 컬럼 갯수에 맞게끔 값을 넣으면 개별 입력 가능

INSERT INTO MEMBER VALUES(NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL); -- NULL값도 지정 가능
...
INSERT INTO MEM_NOTNULL(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE) VALUES(
    2, 'user02', 'pass02', '이진용', SYSDATE);
...
INSERT INTO MEM_UNIQUE(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE) VALUES(
    2, 'user02', 'pass02', '이진용', SYSDATE);
...
INSERT INTO MEM_CHECK(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER) VALUES(
    2, 'user02', 'pass02', '이진용', SYSDATE, '남');
...
INSERT INTO MEM_PRI VALUES(
    1, 'user01', 'pass01', '윤의진', '남', '010-1111-2222', 'aaa@naver.com', '25/06/05'
);
INSERT INTO MEM_PRI(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER) VALUES(
    2, 'user02', 'pass02', '이진용', SYSDATE, '남');
...
-- 시퀀스 생성 / 1로 시작, 1씩 증가, 사이클은 없음

INSERT INTO MEM_PRI VALUES(
    MEM_SEQ.NEXTVAL, 'user01', 'pass01', '윤의진', '남', '010-1111-2222', 'aaa@naver.com', '25/06/05'
);
INSERT INTO MEM_PRI(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER) VALUES(
    MEM_SEQ.NEXTVAL, 'user02', 'pass02', '이진용', SYSDATE, '남');
...
SELECT MEM_ID, MEM_NAME, GRADE_NAME FROM MEM_MEMBER
...
-- SELECT MEM_ID, MEM_NAME, GRADE_NAME FROM MEM_MEMBER2
-- LEFT JOIN MEM_GRADE ON GRADE_ID = GRADE_CODE;
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지 (상황에 따라 각각 이용)
...
-- 테이블명 : BOOK
-- 컬럼 : BK_NO - 숫자, 기본키 / BK_TITLE - 문자(50), NULL허용X / BK_AUTHOR - 문자(50), NULL허용X 
--       PUB_NO - 숫자, 외래키(PUBLISHER의 PUB_NO, 삭제 조건 CASCADE)

-- 테이블명 : MEMBER (위에 만든 건 DROP 하기)
-- 컬럼 : MEMBER_NO - 숫자, 기본키 / MEMBER_ID - 문자(50), 중복 허용X

-- 테이블명 : RENT
-- 컬럼 : RENT_NO - 숫자, 기본키 / MEMBER_NO - 숫자, 외래키 (MEMBER의 MEMBER_NO, 삭제 조건 SET NULL)
...
-- PRIMARY KEY를 끌고 오는 게 중요하다.
-- 각 키값 연동 시에는 JOIN을 이용한다!
...
INSERT INTO BOOK(BK_TITLE,BK_AUTHOR,PUB_NO) VALUES('게으르게 살지만 부자는 되고 싶어','예프리',2);
...
-- RENT 분석 : MEMBER_ID, BK_TITLE, PUB_NAME 값이 필요하다.
SELECT MEMBER_ID, BK_TITLE, PUB_NAME
FROM RENT R
JOIN MEMBER M ON(R.MEMBER_NO = M.MEMBER_NO) -- 별칭이 없을 경우 그대로, RENT.MEMBER_NO = MEMBER.MEMBER_NO
...
-- 변경하려는 자료형의 데이터가 존재할 시 (기존 값과 다른 형태로) 바꾸려고 해서 에러!
ALTER TABLE USER_INFO MODIFY ADDRESS NUMBER;

-- 입력된 값이 없으면 데이터타입 자유롭게 변경 가능!
...
-- 기본값이 변경되는 시점 이후에 추가되는 NULL값에 대해서만 디폴트 값이 적용된다. (이전에 입력된 기본값은 이미 입력된 값으로 취급함)
...
/*
    ...
    *NOT NULL : MODIFY 컬럼명 [CONSTRAINT 제약조건명] NOT NULL; -- ADD가 아님
*/
CREATE TABLE DEPT_COPY
AS SELECT * FROM DEPARTMENT; -- 그대로 복사해오기
...
*/
ALTER TABLE DEPT_COPY
RENAME CONSTRAINT SYS_C008434 TO DEPT_ID_PK;

/*
    3. 테이블명 변경
    ALTER TABLE 기존테이블명 RENAME TO 변경할테이블명;
    ALTER TABLE RENAME 기존테이블명 TO 변경할테이블명; -- 동일
*/
ALTER TABLE DEPT_COPY RENAME TO DEPT_TEST;
SELECT * FROM DEPT_COPY; -- 테이블명 변경되어 조회 불가
...
-- MySQL에서는 오라클과 달리 Schema가 따로 구분되어 있다.
-- 프로젝트를 진행할 때마다 스키마를 새로 추가하여 활용할 것. 생성 이후 더블 클릭을 통한 접속 필요함.