...
public class B_Time {
	...
	public void method1() {
		LocalDateTime now = LocalDateTime.now(); // 현재 지금 날짜
		...
		System.out.println("ZonedDateTime : " + ZonedDateTime.now()); // 현재 위치한 기준 지역 정보 포함 (Asia/Seoul)
		
		// 년, 월, 일, 요일, 시, 분, 초
		System.out.println(now.getYear() + "년");
		System.out.println(now.getMonthValue() + "월(" + now.getMonth() + ")"); // 6월(JUNE)
		...
		// 날짜 조작
		LocalDateTime plusDay = now.plusDays(1).plusMonths(2).plusYears(1); // 1년 2개월 1일 뒤
		System.out.println("plus : " + plusDay);
		
		LocalDateTime minusDay = now.minusYears(1).minusMonths(1).minusDays(5); // 1년 1개월 5일 전
		System.out.println("minus : " + minusDay);
		
		LocalDateTime withDay = now.withYear(2026).withMonth(8).withDayOfMonth(15); // 날짜값 강제 지정
		System.out.println("with : " + withDay);
		
		System.out.println("isAfter : " + now.isAfter(withDay)); // 오늘(앞)이 해당 날짜(뒤)보다 이후인가?
		System.out.println("isBefore : " + now.isBefore(withDay)); // 오늘(앞)이 해당 날짜(뒤)보다 이전인가?
		
		// LocalDate : 날짜 정보 저장
		LocalDate localDate = LocalDate.now();
		System.out.println("LocalDate : " + localDate); // 현재 날짜만 추출
	...
	public void method2() {
		...
		System.out.println("minute : " + du.toMinutesPart()); // -Part가 없는 경우 전체에 대한 분,초로 나옴
...
//		Period pe = Period.between(today, dday);
//		System.out.println(pe);
//		System.out.println("year : " + pe.get(ChronoUnit.YEARS));
//		System.out.println("month : " + pe.get(ChronoUnit.MONTHS));
//		System.out.println("day : " + pe.get(ChronoUnit.DAYS));
		
//		long dDay = ChronoUnit.DAYS.between(today, dday);
//		dDay = date.until(now, ChronoUnit.DAYS);
		
		if(ChronoUnit.DAYS.between(today, dday)<0) {
			System.out.println("D-Day 카운트 : +" + -(ChronoUnit.DAYS.between(today, dday)) + "일");
		} else if (ChronoUnit.DAYS.between(today, dday)==0) {
