...
public class Create {
	...
	public void method1() {
		...
		// range : 첫 번째 파라미터 값 ~ 두 번째 파라미터 값 이전까지의 값을 요소로 가지는 스트림 객체 생성
		IntStream stream = IntStream.range(1, 10); // 1~9 까지 범위 설정
		int sum = stream.sum();
		System.out.println("range의 sum : " + sum);
		
		// rangerClosed : 첫 번째 파라미터 값 ~ 두 번째 파라미터 값까지의 값을 요소로 가지는 스트림 객체 생성
		stream = IntStream.rangeClosed(1, 10); // 1~10 까지 범위 설정
		sum = stream.sum();
	...
	public void method2() {
		...
		// stream.forEach(name -> System.out.println(name)); // 한 번 소진한 이후 재실행 시 불가
