...
/*
 * 매칭
 * - 최종 처리 단계에서 요소들이 특정 조건에 만족하는 지 조사하는 역할
 * - allMatch : 모든 요소들이 매개 값으로 주어진 조건을 만족하는 지 조사
 * - anyMatch : 최소한 한 개의 요소가 매개 값으로 주어진 조건을 만족하는 지 조사
 * - noneMatch : 모든 요소들이 매개 값으로 주어진 조건을 만족하지 않는 지 조사
 * */
public class A_Match {
	...
	public void method2() {
		...
		// 나이가 18살 이하인 학생들이 모두 남자인 지 확인
		boolean result1 = students.stream()
				.filter(student -> student.getAge()<=18)
				.allMatch(student -> student.getGender().equals("남자"));
		System.out.println("나이가 18살 이하인 학생들이 모두 남자인가? " + result1);
				
		// 남학생들 중에 두 과목의 평균이 90점 이상이 한 명이라도 존재하는 지 확인
		boolean result2 = students.stream()
				.filter(student -> student.getGender().equals("남자"))
				.anyMatch(student -> (student.getMath() + student.getEnglish())/2 >=90);
		System.out.println("남학생들 중에 두 과목의 평균이 90점 이상이 한 명이라도 존재하는가? " + result2);
