...
public class A_String {
	...
	public void method1() {
		String str1 = "hello"; // 문자열 그 자체
		String str2 = "hello"; // -> sello로 처럼 값의 일부만 변경하고 싶은 경우, str2="sello"로 초기화해야함.
		String str3 = new String("hello"); // 주소값
		String str4 = new String("hello");
		
		System.out.println("str1 == str2 : " + (str1 == str2)); // true
		System.out.println("str3 == str4 : " + (str3 == str4)); // false : 객체 생성 시 새로운 주소값 생성
		System.out.println("str1 == str3 : " + (str1 == str3)); // false
		
		System.out.println("str1.equals(str2) : " + (str1.equals(str2))); // true
		System.out.println("str3.equals(str4) : " + (str3.equals(str4))); // true
		System.out.println("str1.equals(str3) : " + (str1.equals(str3))); // true (값을 비교하기 위해서는 ==가 아닌 .equals 사용할 것)
	...
	public void method2() {
		...
		// str2 = str + "!!!!!"; - (+)연산자로 대체할 수 있다보니 자주 쓰이지 않음 
		System.out.println("2. concat : " + str2);
		
		// 3. substring(int beginIndex, int endIndex) : String
		//	  문자열의 beginIndex부터 endIndex-1 까지 (endIndex 생략하면 끝까지)의 문자열을 새로 생성해서 리턴
		String str3 = str.substring(7);
		String str4 = str.substring(0, 5);		
		System.out.println("3. substring : " + str3 + str4);
		...
		System.out.println(index2); // 문자열이 올 경우, 맨 앞에 위치한 값을 기준으로 인덱스 값을 출력한다.
		int index3 = str.indexOf("?");
		System.out.println(index3); // 해당 문자열 내 존재하지 않는 값일 경우 -1 출력 
		...
		List<char[]> arrList2 = Arrays.asList(arr);
		...
		System.out.println(str); // 변형을 해도 원래 값 자체가 변형되지는 않는다.
	}		
		/*
		...
		public void method4() {	
			...
			// 리턴 이후에도 값이 그대로 남아 있음(불변)
			
			// 2. StringTokenizer 객체 이용 (Iterator와 유사)
			StringTokenizer st = new StringTokenizer(str,",");
			// hasMoreTokens : 남아 있는 토큰이 있는 지
			System.out.println(st.hasMoreTokens());
			System.out.println("과목 개수 : " + st.countTokens());
			// nextToken : 토큰을 하나씩 꺼내옴
			while(st.hasMoreTokens()) {
				System.out.println(st.nextToken());
			} // 남은 토큰이 있을 때까지 하나씩 꺼냄을 반복
			System.out.println("과목 개수 : " + st.countTokens());
			// 모든 토큰을 빼낸 이후에는 값이 변경된다
		}
