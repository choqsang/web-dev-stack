 ...
 * 		- 메모리 효율
 * 4. 운영체제에 독립적 (JVM)
 * 		- 자바 코드는 JVM 위에서 돌아가기 때문에 Windows, masOS, Linux 모두 실행 가능
 ...
 *   		- Java API (lib/src.zip)	기본적으로 제공되는 다양한 함수들을 이용 가능하다.
...
public class Hello { // 클래스명은 클래스 파일명과 동일해야 한다.
	...
	public static void main(String[] args) {
		...
		System.out.println("Hello, World!"); // 콘솔 창에 출력 (Ctrl+F11로 명령어 실행할 수 있음.)
		
		/*
		 * 네이밍 규칙
		 * - 패키지명 : 소문자로 작성, 회사들 간에 패키지가 서로 중복되지 않도록 회사의 도메인 이름(역순)으로 패키지를 만든다.
		 * - 클래스명 : 대문자로 시작, 여러 개의 단어가 있을 때에는 단어 앞자리마다 대문자로
		 * - 메서드명 : 소문자로 시작, 여러 개의 단어가 있을 때에는 단어 앞자리마다 대문자로
		 * */
	}
