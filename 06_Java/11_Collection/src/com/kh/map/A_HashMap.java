...
/*
 * Map
 * - key - value 쌍(묶음)으로 저장 : 다이아몬드 연산자 내 여러 값 기재 필요
 * - 순서 없음
...
public class A_HashMap {
	...
	public void method1() {
		...
		map.put("웨하스", 250);
		map.put("고래밥", 100);
		map.put("웨하스", 300); // 키는 중복 불가하여 덮어쓴다! (값 수정)
		map.put("칸쵸", 100); // 값은 중복 허용되어 노출
	...
	public void method2() {
		...
		// 웨하스의 맛과 칼로리는?
		System.out.println("웨하스의 맛은 " + map.get("웨하스").getFlavor()+ "맛이고, "
				+ "칼로리는 " + map.get("웨하스").getCalorie() + "kcal 입니다.");
		...
		} System.out.println("'딸기' 맛이 포함된 과자들의 칼로리 총합은 "+ sum + "kcal이고, "
				+ "평균 값은 " + sum/count + "kcal입니다.");
		
		Set<Entry<String, Snack>> entry = map.entrySet();
		for(Entry<String, Snack> e : entry) {
			//System.out.println(e.getKey() + " - " + e.getValue());
		}
	}
